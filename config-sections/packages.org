...
#+end_src
* =magit=
Auto-refresh when I switch to magit
#+begin_src elisp :results none
...
=C-h= to delete words just like with other modes
#+begin_src elisp :results none
...
=org-fancy-priorities= makes it slower to open =org-mode= files without really giving much value
#+begin_src elisp :results none
...
Fix "[fd error]: Malformed pattern in custom ignore file. .project: No such file or directory (os error 2)."
#+begin_src elisp
